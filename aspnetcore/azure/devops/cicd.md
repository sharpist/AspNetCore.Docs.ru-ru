...
1. В раскрывающемся списке **Ветвь по умолчанию для сборок вручную и по расписанию** выберите ветвь по умолчанию (*master*). Нажмите кнопку **Continue** (Продолжить). Откроется страница выбора шаблона.
...
1. Установите флажок **Включить непрерывную интеграцию**. В разделе **Фильтры ветвей** убедитесь в том, что в раскрывающемся списке **Тип** выбран пункт *Включить*. В раскрывающемся списке **Спецификация ветви** выберите пункт *главная*.
    ...
    В результате установки этих параметров сборка будет происходить при отправке любого изменения в ветвь по умолчанию (*master*) репозитория GitHub. Тестирование непрерывной интеграции производится в разделе [Фиксация изменений в GitHub и автоматическое развертывание в Azure](#commit-changes-to-github-and-automatically-deploy-to-azure).
...
1. В разделе **Фильтры ветви сборки** откройте раскрывающийся список **Добавить**. Выберите пункт **Ветвь по умолчанию в определении сборки**. При применении этого фильтра выпуск активируется только для сборки из ветви по умолчанию (*master*) репозитория GitHub.
...
1. Отправьте изменение в ветвь по умолчанию (*master*) удаленного репозитория *origin* в GitHub. В следующей команде вместо заполнители `{BRANCH}` укажите ветвь по умолчанию (используйте `master`):

    ```console
    git push origin {BRANCH}
    ```

    Фиксация появится в ветви по умолчанию (*master*) репозитория GitHub.
