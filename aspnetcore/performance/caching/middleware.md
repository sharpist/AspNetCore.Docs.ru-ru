...
* Cache [-Control](https://tools.ietf.org/html/rfc7234#section-5.2) &ndash; кэширует кэшированные ответы до 10 секунд.
* Параметр [vary](https://tools.ietf.org/html/rfc7231#section-7.1.4) &ndash; позволяет настроить по промежуточного слоя для обработки кэшированного ответа только в том случае, если заголовок [Accept-Encoding](https://tools.ietf.org/html/rfc7231#section-5.3.4) последующих запросов соответствует исходному запросу.
...
При использовании контроллеров MVC, веб-API или Razor Pagesных моделей [`[ResponseCache]`](xref:Microsoft.AspNetCore.Mvc.ResponseCacheAttribute) страниц атрибут задает параметры, необходимые для установки соответствующих заголовков для кэширования ответов. Единственный параметр `[ResponseCache]` атрибута, для которого строго требуется по промежуточного слоя <xref:Microsoft.AspNetCore.Mvc.ResponseCacheAttribute.VaryByQueryKeys>, не соответствует фактическому заголовку HTTP. Для получения дополнительной информации см. <xref:performance/caching/response#responsecache-attribute>.

Если `[ResponseCache]` атрибут не используется, кэширование ответов можно изменять с помощью `VaryByQueryKeys`. Используйте <xref:Microsoft.AspNetCore.ResponseCaching.ResponseCachingFeature> непосредственно из [функции HttpContext. Features](xref:Microsoft.AspNetCore.Http.HttpContext.Features):
...
| `Cache-Control` | По промежуточного слоя рассматривает только ответы кэширования, отмеченные директивой `public` Cache. Управление кэшированием со следующими параметрами:<ul><li>максимальный возраст</li><li>максимальный — устаревший&#8224;</li><li>min-свежая</li><li>must-revalidate</li><li>no-cache</li><li>без магазина</li><li>только в случае кэширования</li><li>private</li><li>public</li><li>s-maxage</li><li>прокси — повторная проверка&#8225;</li></ul>&#8224;если ограничение не задано `max-stale`, по промежуточного слоя не выполняет никаких действий.<br>&#8225;`proxy-revalidate` имеет тот же результат, `must-revalidate`что и.<br><br>Дополнительные сведения см. в статье [RFC 7231: запрос директив управления Cache-Control](https://tools.ietf.org/html/rfc7234#section-5.2.1). |
| `Pragma` | `Pragma: no-cache` Заголовок в запросе дает тот же результат, что `Cache-Control: no-cache`и. Этот заголовок переопределяется соответствующими директивами в `Cache-Control` заголовке, если он имеется. Рассматривается для обеспечения обратной совместимости с HTTP/1.0. |
...
При тестировании и устранении неполадок кэширования браузер может задавать заголовки запросов, которые влияют на кэширование нежелательным образом. Например, браузер может задать для `Cache-Control` `no-cache` заголовка или `max-age=0` при обновлении страницы. Следующие средства могут явно задавать заголовки запроса и являются предпочтительными для тестирования кэширования.
...
* `Pragma: no-cache` Заголовок не должен присутствовать, если `Cache-Control` заголовок отсутствует, так как `Cache-Control` заголовок переопределяет `Pragma` заголовок при его наличии.
...
* Ответ не должен быть устаревшим, как указано в `Expires` заголовках и `max-age` директивах кэша и `s-maxage` .
...
> [!NOTE]
> Система защиты от подделки для создания безопасных маркеров для предотвращения подделки межсайтовых запросов (CSRF) устанавливает заголовки `Cache-Control` и `Pragma` так, `no-cache` чтобы ответы не кэшируются. Сведения об отключении маркеров подделки для элементов HTML-форм см. в разделе <xref:security/anti-request-forgery#aspnet-core-antiforgery-configuration>.
