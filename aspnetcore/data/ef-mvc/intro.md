...
* Запустите Visual Studio и выберите **Веб-приложение ASP.NET Core**  > **Далее**.
* Задайте для проекта имя `ContosoUniversity`. Очень важно использовать именно такое имя с учетом регистра символов, чтобы пространства имен совпадали при копировании кода.
* Щелкните **Создать**.
* Выберите в раскрывающихся списках пункты **.NET Core** и **ASP.NET Core 5.0** , а затем — шаблон **Веб-приложение (модель-представление-контроллер)** .
  ![Диалоговое окно "Создание проекта ASP.NET Core"](intro/_static/new-aspnet5.png)
