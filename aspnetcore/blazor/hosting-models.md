...
После выбора шаблона **Приложение Blazor WebAssembly** можно настроить приложение для использования серверной части ASP.NET Core, установив флажок **С размещением в ASP.NET Core** ([`dotnet new blazorwasm --hosted`](/dotnet/core/tools/dotnet-new)). Приложение ASP.NET Core предоставляет клиентам приложение Blazor. Приложение Blazor WebAssembly может взаимодействовать с сервером по сети с помощью вызовов веб-API или [SignalR](xref:signalr/introduction) (<xref:tutorials/signalr-blazor-webassembly>).
