---
title: 'Защита приложений ASP.NET Core :::no-loc(Blazor Server):::'
author: guardrex
description: 'Сведения о защите приложений :::no-loc(Blazor Server)::: как приложений ASP.NET Core.'
...
---
# <a name="secure-aspnet-core-no-locblazor-server-apps"></a><span data-ttu-id="9022d-103">Защита приложений ASP.NET Core :::no-loc(Blazor Server):::</span><span class="sxs-lookup"><span data-stu-id="9022d-103">Secure ASP.NET Core :::no-loc(Blazor Server)::: apps</span></span>
...
<span data-ttu-id="9022d-105">Приложения :::no-loc(Blazor Server)::: настраиваются для обеспечения безопасности так же, как приложения ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="9022d-105">:::no-loc(Blazor Server)::: apps are configured for security in the same manner as ASP.NET Core apps.</span></span> <span data-ttu-id="9022d-106">Дополнительные сведения см. в статьях в документе <xref:security/index>.</span><span class="sxs-lookup"><span data-stu-id="9022d-106">For more information, see the articles under <xref:security/index>.</span></span> <span data-ttu-id="9022d-107">Разделы в этом обзоре посвящены :::no-loc(Blazor Server):::.</span><span class="sxs-lookup"><span data-stu-id="9022d-107">Topics under this overview apply specifically to :::no-loc(Blazor Server):::.</span></span>

## <a name="no-locblazor-server-project-template"></a><span data-ttu-id="9022d-108">Шаблон проекта :::no-loc(Blazor Server):::</span><span class="sxs-lookup"><span data-stu-id="9022d-108">:::no-loc(Blazor Server)::: project template</span></span>

<span data-ttu-id="9022d-109">Шаблон проекта :::no-loc(Blazor Server)::: можно настроить для проверки подлинности при создании проекта.</span><span class="sxs-lookup"><span data-stu-id="9022d-109">The :::no-loc(Blazor Server)::: project template can be configured for authentication when the project is created.</span></span>
...
<span data-ttu-id="9022d-111">Следуйте инструкциям по работе с Visual Studio (<xref:blazor/tooling>), чтобы создать проект :::no-loc(Blazor Server)::: с механизмом проверки подлинности.</span><span class="sxs-lookup"><span data-stu-id="9022d-111">Follow the Visual Studio guidance in <xref:blazor/tooling> to create a new :::no-loc(Blazor Server)::: project with an authentication mechanism.</span></span>

<span data-ttu-id="9022d-112">Выбрав шаблон **Приложение :::no-loc(Blazor Server):::** в диалоговом окне **Создание веб-приложения ASP.NET Core** , щелкните **Изменить** в разделе **Проверка подлинности** .</span><span class="sxs-lookup"><span data-stu-id="9022d-112">After choosing the **:::no-loc(Blazor Server)::: App** template in the **Create a new ASP.NET Core Web Application** dialog, select **Change** under **Authentication** .</span></span>
...
* <span data-ttu-id="9022d-115">**Учетные записи отдельных пользователей** . Учетные записи пользователей могут храниться:</span><span class="sxs-lookup"><span data-stu-id="9022d-115">**Individual User Accounts** : User accounts can be stored:</span></span>
  * <span data-ttu-id="9022d-116">Внутри приложения с помощью системы [:::no-loc(Identity):::](xref:security/authentication/identity) в ASP.NET Core.</span><span class="sxs-lookup"><span data-stu-id="9022d-116">Within the app using ASP.NET Core's [:::no-loc(Identity):::](xref:security/authentication/identity) system.</span></span>
...
<span data-ttu-id="9022d-121">Следуйте инструкциям по работе с Visual Studio Code (<xref:blazor/tooling>), чтобы создать проект :::no-loc(Blazor Server)::: с механизмом проверки подлинности.</span><span class="sxs-lookup"><span data-stu-id="9022d-121">Follow the Visual Studio Code guidance in <xref:blazor/tooling> to create a new :::no-loc(Blazor Server)::: project with an authentication mechanism:</span></span>
...
| `Individual`             | <span data-ttu-id="9022d-127">Пользователи, сохраненные в приложении с помощью :::no-loc(ASP.NET Core Identity):::</span><span class="sxs-lookup"><span data-stu-id="9022d-127">Users stored in the app with :::no-loc(ASP.NET Core Identity):::</span></span> |
...
1. <span data-ttu-id="9022d-138">На шаге **Настройка нового приложения :::no-loc(Blazor Server):::** выберите **Индивидуальная проверка подлинности (в приложении)** из раскрывающегося списка **Проверка подлинности** .</span><span class="sxs-lookup"><span data-stu-id="9022d-138">On the **Configure your new :::no-loc(Blazor Server)::: App** step, select **Individual Authentication (in-app)** from the **Authentication** drop down.</span></span>

1. <span data-ttu-id="9022d-139">Приложение будет создано для отдельных пользователей, сохраненных в приложении с помощью :::no-loc(ASP.NET Core Identity):::.</span><span class="sxs-lookup"><span data-stu-id="9022d-139">The app is created for individual users stored in the app with :::no-loc(ASP.NET Core Identity):::.</span></span>
...
<span data-ttu-id="9022d-141">Создайте новый проект :::no-loc(Blazor Server)::: с механизмом проверки подлинности, используя следующую команду в командной оболочке:</span><span class="sxs-lookup"><span data-stu-id="9022d-141">Create a new :::no-loc(Blazor Server)::: project with an authentication mechanism using the following command in a command shell:</span></span>
...
| `Individual`             | <span data-ttu-id="9022d-147">Пользователи, сохраненные в приложении с помощью :::no-loc(ASP.NET Core Identity):::</span><span class="sxs-lookup"><span data-stu-id="9022d-147">Users stored in the app with :::no-loc(ASP.NET Core Identity):::</span></span> |
...
* <span data-ttu-id="9022d-157">Выполните команду help для шаблона :::no-loc(Blazor Server)::: (`blazorserver`) в командной оболочке:</span><span class="sxs-lookup"><span data-stu-id="9022d-157">Execute the help command for the :::no-loc(Blazor Server)::: template (`blazorserver`) in a command shell:</span></span>
...
## <a name="scaffold-no-locidentity"></a><span data-ttu-id="9022d-158">Формирование шаблонов :::no-loc(Identity):::</span><span class="sxs-lookup"><span data-stu-id="9022d-158">Scaffold :::no-loc(Identity):::</span></span>

<span data-ttu-id="9022d-159">Сформируйте шаблоны :::no-loc(Identity)::: для проекта :::no-loc(Blazor Server)::::</span><span class="sxs-lookup"><span data-stu-id="9022d-159">Scaffold :::no-loc(Identity)::: into a :::no-loc(Blazor Server)::: project:</span></span>
