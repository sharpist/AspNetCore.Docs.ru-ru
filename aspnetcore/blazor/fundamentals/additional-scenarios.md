...
---
# <a name="aspnet-core-no-locblazor-hosting-model-configuration"></a><span data-ttu-id="aeaa9-103">Конфигурация модели размещения ASP.NET Core :::no-loc(Blazor):::</span><span class="sxs-lookup"><span data-stu-id="aeaa9-103">ASP.NET Core :::no-loc(Blazor)::: hosting model configuration</span></span>
...
### <a name="no-locsignalr-cross-origin-negotiation-for-authentication"></a><span data-ttu-id="aeaa9-106">Согласование независимо от источника для проверки подлинности для :::no-loc(SignalR):::</span><span class="sxs-lookup"><span data-stu-id="aeaa9-106">:::no-loc(SignalR)::: cross-origin negotiation for authentication</span></span>

<span data-ttu-id="aeaa9-107">*Этот раздел относится к :::no-loc(Blazor WebAssembly):::.*</span><span class="sxs-lookup"><span data-stu-id="aeaa9-107">*This section applies to :::no-loc(Blazor WebAssembly):::.*</span></span>

<span data-ttu-id="aeaa9-108">Чтобы настроить базовый клиент :::no-loc(SignalR)::: для отправки учетных данных, таких как файлы :::no-loc(cookie)::: или заголовки проверки подлинности HTTP:</span><span class="sxs-lookup"><span data-stu-id="aeaa9-108">To configure :::no-loc(SignalR):::'s underlying client to send credentials, such as :::no-loc(cookie):::s or HTTP authentication headers:</span></span>
...
<span data-ttu-id="aeaa9-113">*Этот раздел относится к :::no-loc(Blazor Server):::.*</span><span class="sxs-lookup"><span data-stu-id="aeaa9-113">*This section applies to :::no-loc(Blazor Server):::.*</span></span>
...
<span data-ttu-id="aeaa9-116">Чтобы настроить пользовательский интерфейс, определите значение `id` элемента `components-reconnect-modal` в блоке `<body>` страницы :::no-loc(Razor)::: `_Host.cshtml`.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-116">To customize the UI, define an element with an `id` of `components-reconnect-modal` in the `<body>` of the `_Host.cshtml` :::no-loc(Razor)::: page:</span></span>
...
| `components-reconnect-failed`   | <span data-ttu-id="aeaa9-126">Сбой повторного подключения, возможно, из-за сбоя сети.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-126">Reconnection failed, probably due to a network failure.</span></span> <span data-ttu-id="aeaa9-127">Чтобы повторить попытку подключения, вызовите метод `window.:::no-loc(Blazor):::.reconnect()`.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-127">To attempt reconnection, call `window.:::no-loc(Blazor):::.reconnect()`.</span></span> |
...
<span data-ttu-id="aeaa9-137">*Этот раздел относится к :::no-loc(Blazor Server):::.*</span><span class="sxs-lookup"><span data-stu-id="aeaa9-137">*This section applies to :::no-loc(Blazor Server):::.*</span></span>

<span data-ttu-id="aeaa9-138">Приложения :::no-loc(Blazor Server)::: по умолчанию настроены на предварительную отрисовку пользовательского интерфейса на сервере, прежде чем будет установлено подключение клиента с сервером.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-138">:::no-loc(Blazor Server)::: apps are set up by default to prerender the UI on the server before the client connection to the server is established.</span></span> <span data-ttu-id="aeaa9-139">Настройка выполняется на странице :::no-loc(Razor)::: `_Host.cshtml`.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-139">This is set up in the `_Host.cshtml` :::no-loc(Razor)::: page:</span></span>
...
* <span data-ttu-id="aeaa9-142">компонент отображается как статический HTML на странице или включает необходимые сведения для начальной загрузки приложения :::no-loc(Blazor)::: из агента пользователя.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-142">Is rendered as static HTML on the page or if it includes the necessary information to bootstrap a :::no-loc(Blazor)::: app from the user agent.</span></span>
...
| --- | --- |
| <xref:Microsoft.AspNetCore.Mvc.Rendering.RenderMode.ServerPrerendered> | <span data-ttu-id="aeaa9-145">Преобразует компонент в статический HTML и включает метку приложения :::no-loc(Blazor Server):::.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-145">Renders the component into static HTML and includes a marker for a :::no-loc(Blazor Server)::: app.</span></span> <span data-ttu-id="aeaa9-146">При запуске пользовательского агента эта метка используется для начальной загрузки приложения :::no-loc(Blazor):::.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-146">When the user-agent starts, this marker is used to bootstrap a :::no-loc(Blazor)::: app.</span></span> |
| <xref:Microsoft.AspNetCore.Mvc.Rendering.RenderMode.Server> | <span data-ttu-id="aeaa9-147">Отображает метку приложения :::no-loc(Blazor Server):::.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-147">Renders a marker for a :::no-loc(Blazor Server)::: app.</span></span> <span data-ttu-id="aeaa9-148">Выходные данные компонента не включаются.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-148">Output from the component isn't included.</span></span> <span data-ttu-id="aeaa9-149">При запуске пользовательского агента эта метка используется для начальной загрузки приложения :::no-loc(Blazor):::.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-149">When the user-agent starts, this marker is used to bootstrap a :::no-loc(Blazor)::: app.</span></span> |
...
## <a name="initialize-the-no-locblazor-circuit"></a><span data-ttu-id="aeaa9-152">Инициализация канала :::no-loc(Blazor):::</span><span class="sxs-lookup"><span data-stu-id="aeaa9-152">Initialize the :::no-loc(Blazor)::: circuit</span></span>

<span data-ttu-id="aeaa9-153">*Этот раздел относится к :::no-loc(Blazor Server):::.*</span><span class="sxs-lookup"><span data-stu-id="aeaa9-153">*This section applies to :::no-loc(Blazor Server):::.*</span></span>

<span data-ttu-id="aeaa9-154">Настройте ручной запуск [ канала :::no-loc(SignalR):::](xref:blazor/hosting-models#circuits) приложения :::no-loc(Blazor Server)::: в файле `Pages/_Host.cshtml`:</span><span class="sxs-lookup"><span data-stu-id="aeaa9-154">Configure the manual start of a :::no-loc(Blazor Server)::: app's [:::no-loc(SignalR)::: circuit](xref:blazor/hosting-models#circuits) in the `Pages/_Host.cshtml` file:</span></span>
...
* <span data-ttu-id="aeaa9-156">Разместите скрипт, который вызывает `:::no-loc(Blazor):::.start` после тега скрипта `blazor.server.js` и внутри закрывающего тега `</body>`.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-156">Place a script that calls `:::no-loc(Blazor):::.start` after the `blazor.server.js` script's tag and inside the closing `</body>` tag.</span></span>

<span data-ttu-id="aeaa9-157">При отключении `autostart` все части приложения, которые не зависят от канала, работают нормально.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-157">When `autostart` is disabled, any aspect of the app that doesn't depend on the circuit works normally.</span></span> <span data-ttu-id="aeaa9-158">Например, работает маршрутизация на стороне клиента.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-158">For example, client-side routing is operational.</span></span> <span data-ttu-id="aeaa9-159">Однако все аспекты, зависящие от канала, не будут работать до вызова `:::no-loc(Blazor):::.start`.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-159">However, any aspect that depends on the circuit isn't operational until `:::no-loc(Blazor):::.start` is called.</span></span> <span data-ttu-id="aeaa9-160">Пока не будет установлен канал, поведение приложения будет непредсказуемым.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-160">App behavior is unpredictable without an established circuit.</span></span> <span data-ttu-id="aeaa9-161">Например, когда канал отключен, методы компонентов не выполняются.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-161">For example, component methods fail to execute while the circuit is disconnected.</span></span>

### <a name="initialize-no-locblazor-when-the-document-is-ready"></a><span data-ttu-id="aeaa9-162">Инициализация :::no-loc(Blazor)::: при готовности документа</span><span class="sxs-lookup"><span data-stu-id="aeaa9-162">Initialize :::no-loc(Blazor)::: when the document is ready</span></span>

<span data-ttu-id="aeaa9-163">Чтобы инициализировать приложение :::no-loc(Blazor):::, когда документ готов, используйте следующий код:</span><span class="sxs-lookup"><span data-stu-id="aeaa9-163">To initialize the :::no-loc(Blazor)::: app when the document is ready:</span></span>
...
<span data-ttu-id="aeaa9-165">Для выполнения дополнительных задач, таких как инициализация взаимодействия JS, используйте `then` для привязки к объекту `Promise`, полученному в результате запуска приложения :::no-loc(Blazor)::: вручную.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-165">To perform additional tasks, such as JS interop initialization, use `then` to chain to the `Promise` that results from a manual :::no-loc(Blazor)::: app start:</span></span>
...
### <a name="configure-the-no-locsignalr-client"></a><span data-ttu-id="aeaa9-166">Настройка клиента :::no-loc(SignalR):::</span><span class="sxs-lookup"><span data-stu-id="aeaa9-166">Configure the :::no-loc(SignalR)::: client</span></span>
...
<span data-ttu-id="aeaa9-168">Чтобы настроить ведение журнала клиента :::no-loc(SignalR):::, передайте объект конфигурации (`configure:::no-loc(SignalR):::`), который вызывает `configureLogging` с уровнем журнала для построителя клиента:</span><span class="sxs-lookup"><span data-stu-id="aeaa9-168">To configure :::no-loc(SignalR)::: client logging, pass in a configuration object (`configure:::no-loc(SignalR):::`) that calls `configureLogging` with the log level on the client builder:</span></span>
...
## <a name="disconnect-the-no-locblazor-circuit-from-the-client"></a><span data-ttu-id="aeaa9-186">Отключение канала :::no-loc(Blazor)::: от клиента</span><span class="sxs-lookup"><span data-stu-id="aeaa9-186">Disconnect the :::no-loc(Blazor)::: circuit from the client</span></span>

<span data-ttu-id="aeaa9-187">По умолчанию канал :::no-loc(Blazor)::: отключается при активации [события страницы `unload`](https://developer.mozilla.org/docs/Web/API/Window/unload_event).</span><span class="sxs-lookup"><span data-stu-id="aeaa9-187">By default, a :::no-loc(Blazor)::: circuit is disconnected when the [`unload` page event](https://developer.mozilla.org/docs/Web/API/Window/unload_event) is triggered.</span></span> <span data-ttu-id="aeaa9-188">Чтобы отключить канал от клиента в других сценариях, вызовите `:::no-loc(Blazor):::.disconnect` в соответствующем обработчике событий.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-188">To disconnect the circuit for other scenarios on the client, invoke `:::no-loc(Blazor):::.disconnect` in the appropriate event handler.</span></span> <span data-ttu-id="aeaa9-189">В следующем примере канал отключается, когда скрывается страница ([событие `pagehide`](https://developer.mozilla.org/docs/Web/API/Window/pagehide_event)):</span><span class="sxs-lookup"><span data-stu-id="aeaa9-189">In the following example, the circuit is disconnected when the page is hidden ([`pagehide` event](https://developer.mozilla.org/docs/Web/API/Window/pagehide_event)):</span></span>
...
<span data-ttu-id="aeaa9-191">*Этот раздел относится к :::no-loc(Blazor Server):::.*</span><span class="sxs-lookup"><span data-stu-id="aeaa9-191">*This section applies to :::no-loc(Blazor Server):::.*</span></span>
...
* <span data-ttu-id="aeaa9-197">Используйте два вызова <xref:Microsoft.AspNetCore.Builder.StaticFileExtensions.UseStaticFiles%2A> в `Startup.Configure` (`Startup.cs`).</span><span class="sxs-lookup"><span data-stu-id="aeaa9-197">Use two calls to <xref:Microsoft.AspNetCore.Builder.StaticFileExtensions.UseStaticFiles%2A> in `Startup.Configure` (`Startup.cs`):</span></span>
  ...
  * <span data-ttu-id="aeaa9-199">Второе ПО промежуточного слоя обслуживает `blazor.server.js`, в котором используется конфигурация статических файлов по умолчанию, предоставляемая платформой :::no-loc(Blazor):::.</span><span class="sxs-lookup"><span data-stu-id="aeaa9-199">The second middleware serves `blazor.server.js`, which uses the default static files configuration provided by the :::no-loc(Blazor)::: framework.</span></span>
