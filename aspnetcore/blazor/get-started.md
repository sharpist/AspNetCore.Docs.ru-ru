...
1. В поле **Имя проекта** укажите имя проекта или оставьте имя по умолчанию. Убедитесь, что для проекта правильно указано существующее **расположение** или укажите новое. Нажмите кнопку **создания**.

1. Для работы с Blazor WebAssembly выберите шаблон **Приложение Blazor WebAssembly**. Для работы с Blazor Server выберите шаблон **Серверное приложение Blazor** . Нажмите кнопку **создания**.
...
На странице Counter нажмите кнопку увеличения значения счетчика без обновления страницы. Обычно для увеличения значений счетчика на веб-странице требуется код JavaScript, однако с Blazor можно использовать C#.
...
Каждый раз при нажатии кнопки выполняются следующие действия:
...
Запустите приложение. Теперь у компонента `Index` будет собственный счетчик, который будет увеличиваться на десять при каждом нажатии кнопки. Компонент `Counter` (`Pages/Counter.razor`) в компоненте `/counter` продолжает увеличиваться на единицу.
