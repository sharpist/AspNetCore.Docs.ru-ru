...
В этом документе описываются представления, используемые в приложениях ASP.NET Core MVC. Сведения о Razor страницах см. [в разделе Введение в Razor страницы](xref:razor-pages/index).

В шаблоне MVC (Model-View-Controller, модель — представление — контроллер) *представление* отвечает за отображение данных приложения и взаимодействие с пользователем. Представление — это шаблон HTML с внедренной [ Razor разметкой](xref:mvc/views/razor). Razor Разметка — это код, который взаимодействует с разметкой HTML для создания веб-страницы, отправляемой клиенту.

В ASP.NET Core MVC представления — это файлы *. cshtml* , которые используют [язык программирования C#](/dotnet/csharp/) в Razor разметке. Как правило, файлы представлений объединяются в папки с именами, соответствующими отдельным [контроллерам](xref:mvc/controllers/actions) приложения. Эти папки находятся в папке *Views* в корне приложения.
...
В [Visual Studio](https://visualstudio.microsoft.com) и [Visual Studio Code](https://code.visualstudio.com/) строго типизированные члены классов перечисляются с помощью функции [IntelliSense](/visualstudio/ide/using-intellisense). Чтобы просмотреть свойства viewmodel, введите имя переменной viewmodel с точкой (`.`) после него. Это позволяет писать код быстрее, допуская меньше ошибок.
...
`ViewBag`*недоступно в Razor Страницы.*
...
| Представлением и [представлением макета](xref:mvc/views/layout)   | Установка **\<title>** содержимого элемента в представлении макета из файла представления.  |
...
Другой подход, который использует [ViewDataDictionary](/dotnet/api/microsoft.aspnetcore.mvc.viewfeatures.viewdatadictionary), — [ViewDataAttribute](/dotnet/api/microsoft.aspnetcore.mvc.viewdataattribute). Значения свойств контроллеров или Razor моделей страниц, помеченных `[ViewData]` атрибутом, хранятся и загружаются из словаря.
...
**Сводка различий между ViewData и ViewBag**
 ...
 `ViewBag` недоступна на Razor страницах.
