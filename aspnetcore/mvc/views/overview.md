...
| Представлением и [представлением макета](xref:mvc/views/layout)   | Задание для ** \<заголовка** содержимого элемента>в представлении макета из файла представления.  |
...
Другой подход, который использует [ViewDataDictionary](/dotnet/api/microsoft.aspnetcore.mvc.viewfeatures.viewdatadictionary), — [ViewDataAttribute](/dotnet/api/microsoft.aspnetcore.mvc.viewdataattribute). Значения свойств контроллеров Razor или моделей страниц, помеченных `[ViewData]` атрибутом, хранятся и загружаются из словаря.
...
* `ViewData`
  * Является производным [от ViewDataDictionary](/dotnet/api/microsoft.aspnetcore.mvc.viewfeatures.viewdatadictionary), поэтому он имеет свойства словаря, которые могут быть полезными `ContainsKey`, `Add`такие `Remove`как, `Clear`, и.
