...
При создании нового веб-приложения ASP.NET Core в Visual Studio добавляется пакет NuGet Microsoft. Razor AspNetCore. Средства». Это пакет добавляет инструментарий вспомогательной функции тега.
...
Символ at () указывает на то, что `@` Razor это начало кода. Следующие два параметра ("FirstName" и "First Name:") представляют собой строки, поэтому [IntelliSense](/visualstudio/ide/using-intellisense) помочь здесь не может. Последний аргумент:

```cshtml
new {@class="caption"}
```

является анонимным объектом, который используется для представления атрибутов. Так как `class` — это зарезервированное ключевое слово в C#, используется символ `@`, чтобы в C# интерпретировать `@class=` как символ (имя свойства). В интерфейсном конструкторе (кто знаком с HTML/CSS/JavaScript и другими клиентскими технологиями, но не знаком с C# и Razor ), большая часть этой строки является внешней. И вся строка должны быть написана без помощи IntelliSense.
