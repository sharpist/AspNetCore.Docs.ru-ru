...
[Стив Смит](https://ardalis.com/) и [Ратгер Шторм](https://github.com/rutix)
...
* Используйте безопасное имя файла, определяемое приложением. Не используйте имя файла, предоставленное пользователем, или недоверчивое имя загружаемого файла. &dagger; HTML кодирует недоверчивое имя файла при его отображении. Например, при записи имени файла в журнал или отображении в пользовательском интерфейсе (Razor автоматически кодирует выходные данные в формате HTML).
...
* Проверить, что проверки на стороне клиента выполняются на сервере. &dagger; Проверки на стороне клиента легко обойти.
...
* Служба хранилища данных (например, хранилище [BLOB-объектов Azure](https://azure.microsoft.com/services/storage/blobs/)).
  ...
  * Затраты на использование служб обычно ниже в сценариях с крупномасштабной инфраструктурой хранения.
  ...
  Для получения дополнительной информации [см.](/azure/storage/blobs/storage-quickstart-blobs-dotnet)
...
* [Физическое хранение](#upload-small-files-with-buffered-model-binding-to-physical-storage)
...
* Любая из следующих коллекций, представляющих несколько файлов:
  ...
  * [Список](xref:System.Collections.Generic.List`1)\<<xref:Microsoft.AspNetCore.Http.IFormFile>>
...
Также `RequestSizeLimitAttribute` может быть применен [`@attribute`](xref:mvc/views/razor#attribute) с помощью директивы Razor:
...
* Служба хранилища данных (например, хранилище [BLOB-объектов Azure](https://azure.microsoft.com/services/storage/blobs/)).
  ...
  * Затраты на использование служб обычно ниже в сценариях с крупномасштабной инфраструктурой хранения.
  ...
  Для получения дополнительной информации [см.](/azure/storage/blobs/storage-quickstart-blobs-dotnet) В этом разделе показан метод <xref:Microsoft.Azure.Storage.File.CloudFile.UploadFromFileAsync*>, но метод <xref:Microsoft.Azure.Storage.File.CloudFile.UploadFromStreamAsync*> можно использовать для сохранения <xref:System.IO.FileStream> в хранилище BLOB-объектов при работе с <xref:System.IO.Stream>.
...
* [Неограниченная отправка файлов](https://owasp.org/www-community/vulnerabilities/Unrestricted_File_Upload)
* [Безопасность Azure: Кадр безопасности: Проверка ввода (ru) Смягчения последствий](/azure/security/azure-security-threat-modeling-tool-input-validation)
* [Шаблоны проектирования облачных технологий Azure: шаблон Ключ валет](/azure/architecture/patterns/valet-key)
