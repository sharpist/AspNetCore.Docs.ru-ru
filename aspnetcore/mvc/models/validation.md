...
Обратите внимание на то, что атрибуты `data-` в выходных данных HTML соответствуют атрибутам проверки для свойства `Movie.ReleaseDate`. Атрибут `data-val-required` содержит сообщение об ошибке, которое выводится, если пользователь не заполнил поле даты выхода. при некорректной проверке JQuery это значение передается в метод, необходимый для проверки JQuery [()](https://jqueryvalidation.org/required-method/) , который затем отображает это сообщение в соответствующем ** \<элементе Span>** .
