...
Для управления конфиденциальными данными конфигурации, например паролями, .NET Core предоставляет [диспетчер секретов](xref:security/app-secrets#secret-manager). Для секретов в рабочей среде рекомендуется использовать [Azure Key Vault](xref:security/key-vault-configuration).
...
Для управления конфиденциальными данными конфигурации, например паролями, .NET Core предоставляет [диспетчер секретов](xref:security/app-secrets). Для секретов в рабочей среде рекомендуется использовать [Azure Key Vault](xref:security/key-vault-configuration).
