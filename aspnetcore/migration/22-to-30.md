...
Как описано в документации по [различиям между предыдущими версиями маршрутизации](xref:fundamentals/routing#differences-from-earlier-versions-of-routing), существуют некоторые различия в создании ссылок ( `Url.Link` например, использование и аналогичные интерфейсы API). К ним относятся следующие объекты.
...
Пакет [Microsoft. Extensions. Caching. Redis](https://www.nuget.org/packages/Microsoft.Extensions.Caching.Redis) недоступен для приложений ASP.NET Core 3,0 и более поздних версий. Замените ссылку на пакет [Microsoft. Extensions. Caching. стаккексчанжередис](https://www.nuget.org/packages/Microsoft.Extensions.Caching.StackExchangeRedis). Дополнительные сведения см. в разделе <xref:performance/caching/distributed>.
