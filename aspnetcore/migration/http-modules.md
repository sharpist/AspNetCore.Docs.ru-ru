...
2. <span data-ttu-id="61dea-185">Сохранение значений параметров</span><span class="sxs-lookup"><span data-stu-id="61dea-185">Store the option values</span></span>
   ...
   <span data-ttu-id="61dea-186">Система конфигурации позволяет хранить значения параметров в любом месте.</span><span class="sxs-lookup"><span data-stu-id="61dea-186">The configuration system allows you to store option values anywhere you want.</span></span> <span data-ttu-id="61dea-187">Однако большинство сайтов используют *:::no-loc(appsettings.json):::* , поэтому мы будем использовать такой подход:</span><span class="sxs-lookup"><span data-stu-id="61dea-187">However, most sites use *:::no-loc(appsettings.json):::* , so we'll take that approach:</span></span>
...
3. <span data-ttu-id="61dea-190">Связывание значений параметров с классом Options</span><span class="sxs-lookup"><span data-stu-id="61dea-190">Associate the option values with the options class</span></span>
   ...
   1. <span data-ttu-id="61dea-193">Если вы используете *:::no-loc(appsettings.json):::* , добавьте его в построитель конфигураций в `Startup` конструкторе:</span><span class="sxs-lookup"><span data-stu-id="61dea-193">If you're using *:::no-loc(appsettings.json):::* , add it to the configuration builder in the `Startup` constructor:</span></span>
...
1. <span data-ttu-id="61dea-209">Добавьте второй ключ в *:::no-loc(appsettings.json):::*</span><span class="sxs-lookup"><span data-stu-id="61dea-209">Add a second key to *:::no-loc(appsettings.json):::*</span></span>

   <span data-ttu-id="61dea-210">Чтобы добавить в файл второй набор параметров *:::no-loc(appsettings.json):::* , используйте новый ключ для уникальной идентификации.</span><span class="sxs-lookup"><span data-stu-id="61dea-210">To add a second set of options to the *:::no-loc(appsettings.json):::* file, use a new key to uniquely identify it:</span></span>
...
<span data-ttu-id="61dea-233">**HttpContext. Request. :::no-loc(Cookie):::** преобразует в:</span><span class="sxs-lookup"><span data-stu-id="61dea-233">**HttpContext.Request.:::no-loc(Cookie):::s** translates to:</span></span>
...
<span data-ttu-id="61dea-260">**HttpContext. Response. :::no-loc(Cookie)::: #d0**</span><span class="sxs-lookup"><span data-stu-id="61dea-260">**HttpContext.Response.:::no-loc(Cookie):::s**</span></span>

<span data-ttu-id="61dea-261">:::no-loc(Cookie):::перемещаются в браузер в заголовке *Set :::no-loc(Cookie)::: -* Response.</span><span class="sxs-lookup"><span data-stu-id="61dea-261">:::no-loc(Cookie):::s travel to the browser in a *Set-:::no-loc(Cookie):::* response header.</span></span> <span data-ttu-id="61dea-262">В результате для отправки сообщений :::no-loc(cookie)::: s требуется тот же ответный вызов, который используется для отправки заголовков ответа:</span><span class="sxs-lookup"><span data-stu-id="61dea-262">As a result, sending :::no-loc(cookie):::s requires the same callback as used for sending response headers:</span></span>
...
<span data-ttu-id="61dea-263">`Set:::no-loc(Cookie):::s`Метод обратного вызова будет выглядеть следующим образом:</span><span class="sxs-lookup"><span data-stu-id="61dea-263">The `Set:::no-loc(Cookie):::s` callback method would look like the following:</span></span>
