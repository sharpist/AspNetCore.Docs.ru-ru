...
## <a name="changes-to-model-binding-in-aspnet-core-mvc-and-no-locrazor-pages"></a>Изменения в привязке модели в ASP.NET Core MVC и Razor страницах

### <a name="datetime-values-are-model-bound-as-utc-times"></a>Значения даты и времени привязаны к модели в формате UTC

В ASP.NET Core 3,1 и более ранних `DateTime` значения были привязаны к модели как местное время, когда часовой пояс был определен сервером. `DateTime` значения, связанные с форматированием входных данных (JSON) и `DateTimeOffset` значениями, были привязаны как часовые пояса в формате UTC. В ASP.NET Core 5,0 и более поздних версиях привязка модели согласованно привязывает `DateTime` значения к часовому поясу UTC.

Чтобы удержать предыдущее поведение, удалите компонент в процессе `DateTimeModelBinderProvider` запуска приложения:
...
В рамках добавления поддержки типов записей привязки модели `ComplexTypeModelBinderProvider` атрибут объявляется как устаревший и больше не регистрируется по умолчанию. Приложения, которые ранее основывались на наличии `ComplexTypeModelBinderProvider` в `ModelBinderProviders` коллекции, должны быть обновлены для ссылки на новый поставщик подшивки:
