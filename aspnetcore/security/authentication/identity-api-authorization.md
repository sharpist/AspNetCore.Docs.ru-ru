...
* Внутри `Startup.ConfigureServices` метода:
  ...
  * Проверка подлинности с помощью дополнительного `AddIdentityServerJwt` вспомогательного метода, который настраивает приложение для проверки маркеров JWT, созданных IdentityServer:
...
В файле *appSettings. JSON* приложения измените `IdentityServer` раздел, включив в него ключевые сведения:
