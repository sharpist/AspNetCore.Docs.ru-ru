...
* Вызывает метод <xref:Microsoft.AspNetCore.Builder.CorsMiddlewareExtensions.UseCors*> расширения и задает политику `_myAllowSpecificOrigins` CORS. `UseCors`добавляет по промежуточного слоя CORS. Вызов `UseCors` должен быть помещен после `UseRouting`, но до. `UseAuthorization` Дополнительные сведения см. в разделе [порядок по промежуточного слоя](xref:fundamentals/middleware/index#middleware-order).
...
* Включает политику `_myAllowSpecificOrigins` CORS для всех конечных точек контроллера. Чтобы применить политику CORS к конкретным конечным точкам, см. раздел [Маршрутизация конечных](#ecors) точек.

При маршрутизации конечных точек по промежуточного слоя CORS ***должно*** быть настроено для выполнения `UseRouting` между `UseEndpoints`вызовами функций и.
...
Примечание. указанный URL-адрес **не** должен содержать косую черту`/`(). Если URL-адрес завершается `/`с, то сравнение `false` возвращает значение и заголовок не возвращается.
...
* Конечные точки контроллера `/echo` и позволяют выполнять запросы между источниками с помощью указанной политики.
* Конечные точки страниц `/echo2` и Razor ***не*** позволяют выполнять запросы независимо от источника, так как не указана политика по умолчанию.

Атрибут [[дисаблекорс]](#dc) ***не*** отключает CORS, который был включен с помощью маршрутизации конечной точки с `RequireCors`помощью.
...
<xref:Microsoft.AspNetCore.Cors.Infrastructure.CorsPolicyBuilder.AllowAnyOrigin*>&ndash; РАЗРЕШАЕТ запросы CORS из всех источников с любой схемой (`http` или `https`). `AllowAnyOrigin`не является безопасным, так как *любой веб-сайт* может выполнять запросы между источниками в приложение.
...
В спецификации CORS также указывается, что при наличии `"*"` `Access-Control-Allow-Credentials` заголовка недопустимыми являются исходные значения (все источники).
...
* [Access-Control-request-headers](https://developer.mozilla.org/docs/Web/HTTP/Headers/Access-Control-Allow-Headers). список заголовков запросов, которые приложение устанавливает на основе фактического запроса. Как упоминалось ранее, в него не входят заголовки, заданных браузером `User-Agent`, например.
...
Браузеры не согласуются с тем `Access-Control-Request-Headers`, как они заданы. Если:
...
* Глобально путем `app.UseCors` вызова `Startup.Configure`в.
...
В `OPTIONS` запросах сервер задает `Access-Control-Allow-Origin: {allowed origin}` заголовок заголовка **ответа** в ответе. Например [, запрос](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/security/cors/3.1sample/Cors/WebAPI)на нажатие кнопки `OPTIONS` [удалить [EnableCors]](https://cors1.azurewebsites.net/test?number=2) содержит следующие заголовки:
...
* Используйте развернутый пример приложения [https://cors3.azurewebsites.net/](https://cors3.azurewebsites.net/)в. Нет необходимости скачивать пример.
...
Кнопки **Delete [EnableCors]** и **Get [EnableCors]** выполняются успешно, так как конечные `[EnableCors]` точки имеют и отвечают на предпечатные запросы. Другие конечные точки не удается. Не удается выполнить кнопку **Get** , так как [JavaScript](https://github.com/dotnet/AspNetCore.Docs/tree/master/aspnetcore/security/cors/3.1sample/Cors/WebAPI/wwwroot/js/MyJS.js) отправляет:
...
См. раздел [Включение Razor CORS на страницах, контроллерах и методах действий](#ecors) для применения политики CORS на уровне страницы, контроллера или действия.
...
С `[EnableCors]` атрибутом можно применить различные политики для контроллера, модели страницы или действия. Если `[EnableCors]` атрибут применяется к контроллеру, модели страницы или методу действия, а CORS включен по промежуточного слоя, применяются ***обе*** политики. ***Не*** рекомендуется объединять политики. Используйте атрибут `[EnableCors]` или по промежуточного слоя, ***не оба**. При использовании `[EnableCors]` **не определяйте политику** по умолчанию.
...
* `Access-Control-Request-Headers`: Список заголовков запросов, которые приложение задает для фактического запроса. Как упоминалось ранее, в него не входят заголовки, заданных браузером `User-Agent`, например.
...
1. Создайте проект веб-приложения (Razor страницы или MVC). В образце Razor используются страницы. Вы можете создать веб-приложение в том же решении, что и проект API.
